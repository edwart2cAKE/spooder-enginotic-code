...
Intake::Intake(pros::Motor &motor, pros::Optical &colorSensor)
    : motor(motor), colorSensor(colorSensor), jammed(false), jam_timer(500),
      anti_jam_timer(1000), color_sort_timer(10), color_sort_stop_timer(200),
      was_ring_detected(false), min_color(-1), max_color(-1),
      task_created(false), desired_voltage(127) {}
...
void Intake::controlIntake() {
  ...
  if (motor.get_actual_velocity() > -10 && motor.get_actual_velocity() < 10 &&
      motor.get_current_draw() > 0.7 * motor.get_current_limit()) {
    jam_timer.resume();
    if (jam_timer.isDone()) {
      jammed = true;
  ...
  if (!task_created) {
    pros::Task([this]() {
      while (true) {
        ...
        controlIntake(); // will stop the motor if jammed or color sort is active automatically
